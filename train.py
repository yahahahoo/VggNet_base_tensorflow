...
def main():
    ...
    train_image_list = glob.glob(train_dir+"/*/*.jpg") # 获取指定路径下的所有图片
    ...
    for i in range(25):
        ...
        # plt.imshow(train_image_list[i], cmap=plt.cm.binary)
        plt.xlabel(inverse_dict[train_label_list[i]])
    # plt.show()
    ...
    # 实例化模型
    model = VggNet("vgg16", 224, 224, 5) # 输入224*224, 输出5
    model.summary()
